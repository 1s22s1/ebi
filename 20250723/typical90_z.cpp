...
int main()
{
    ...
    auto white = count_if(visited.begin(), visited.end(), [](int n)
                          { return n == 0; });
    auto black = count_if(visited.begin(), visited.end(), [](int n)
                          { return n == 1; });
    ll count = 0;

    rep(i, N)
    {
        if (white >= black && visited.at(i) == 0)
        {
            cout << i + 1 << ' ';
            count++;
        }

        if (white < black && visited.at(i) == 1)
        {
            cout << i + 1 << ' ';
            count++;
        }
